import type { Metadata, Viewport } from 'next';
import Script from 'next/script';
import { Inter, Roboto_Mono } from 'next/font/google';

// Cascade order is intentional. Keep legacy-ui immediately after globals until its
// feature selectors have been migrated into their owning stylesheets.
import './globals.css';
import './legacy-ui.css';
import './tokens.css';
import './responsive.css';
import './designsystem2.css';
import './product2.css';
import './pages2.css';
import './host-calendar.css';
import './host-calendar-overrides.css';
import './messages.css';
import './rental-search.css';
import './rental-search-overrides.css';
import './product-title-lines.css';
import './new-listing.css';
import './host-onboarding.css';
import './host-listings.css';
import './auth.css';
import './booking-flow.css';
import './reviews.css';
import './issues.css';
import './admin.css';
import './home3.css';
import './navigation2.css';
import './cross-page-overrides.css';
import './workspace-layout-overrides.css';
import './launch-states.css';
import './referrals.css';

import BottomNav from '@/components/BottomNav';
import GlobalSkipLink from '@/components/GlobalSkipLink';

const inter = Inter({
  subsets: ['latin'],
  weight: ['400', '500', '600', '700', '800', '900'],
  variable: '--font-inter',
  display: 'swap',
});

const mono = Roboto_Mono({
  subsets: ['latin'],
  variable: '--font-mono',
  display: 'swap',
});

export const metadata: Metadata = {
  title: 'Hyrbart',
  description: 'Det du behöver. När du behöver det.',
  manifest: '/manifest.webmanifest?v=6',
  robots: {
    index: false,
    follow: false,
    noarchive: true,
    nosnippet: true,
    nocache: true,
    googleBot: {
      index: false,
      follow: false,
      noimageindex: true,
      nosnippet: true,
    },
  },
  icons: {
    icon: [{ url: '/hyrbart-h-v2.svg?v=6', type: 'image/svg+xml' }],
    shortcut: ['/hyrbart-h-v2.svg?v=6'],
    apple: [{ url: '/apple-touch-icon.png?v=6', sizes: '180x180', type: 'image/png' }],
  },
  appleWebApp: {
    capable: true,
    statusBarStyle: 'black-translucent',
    title: 'Hyrbart',
  },
};

export const viewport: Viewport = {
  width: 'device-width',
  initialScale: 1,
  viewportFit: 'cover',
  themeColor: '#fafafa',
};

export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="sv" className={`${inter.variable} ${mono.variable}`}>
      <body>
        <Script
          id="hyrbart-document-language"
          strategy="beforeInteractive"
          dangerouslySetInnerHTML={{
            __html:
              "(function(){var m=location.pathname.match(/\\/(?:topsecret\\/)?(sv|en)(?:\\/|$)/);if(m)document.documentElement.lang=m[1];})();",
          }}
        />
        <GlobalSkipLink />
        <main id="main-content" tabIndex={-1}>
          {children}
        </main>
        <BottomNav />
      </body>
    </html>
  );
}
