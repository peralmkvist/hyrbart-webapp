...
import { getAdminAccess, type AdminPermission } from '@/lib/admin';
...
const ACTION_PERMISSION:Record<string,AdminPermission>={
  account_status:'users.manage',support_note:'users.manage',risk_flag:'risk.manage',risk_flag_status:'risk.manage',listing_moderation:'risk.manage',
};
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(!permission)return NextResponse.json({error:'UNKNOWN_ACTION'},{status:400});
  const access=await getAdminAccess(permission);
  if(!access)return NextResponse.json({error:'FORBIDDEN'},{status:403});
  const adminUser=access.user;
  const admin=createAdminClient();
  const audit=(entry:{action:string;entityType:string;entityId?:string|null;metadata?:Record<string,unknown>})=>recordAdminAction({...entry,adminUserId:adminUser.id,adminRole:access.role});
  ...
  if(action==='account_status'){
    ...
    await audit({action:'account_status_changed',entityType:'user',entityId:id,metadata:{previous_status:previous,new_status:status,reason:reason||null}});
  ...
  if(action==='support_note'){
    ...
    if(error)throw error;
    await audit({action:'support_note_added',entityType:'user',entityId:id,metadata:{note_id:data.id}});
  ...
  if(action==='risk_flag'){
    ...
    if(error)throw error;
    await audit({action:'risk_flag_created',entityType:'user',entityId:id,metadata:{flag_id:data.id,severity,reason,booking_id:bookingId}});
  ...
  if(action==='risk_flag_status'){
    ...
    await audit({action:'risk_flag_status_changed',entityType:'user',entityId:id,metadata:{flag_id:flagId,previous_status:flag.status,new_status:status}});
    return NextResponse.json({ok:true,flag:data});
  }
  ...
  if(action==='listing_moderation'){
    ...
    if(error)throw error;
    await audit({action:'listing_moderation_changed',entityType:'product',entityId:productId,metadata:{status,reason:reason||null,user_id:id}});
