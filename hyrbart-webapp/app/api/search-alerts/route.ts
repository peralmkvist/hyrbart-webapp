...
function validCriteria(c:ReturnType<typeof canonical>){if(!c.from||!c.to||c.to<c.from)return'INVALID_DATES';if(!c.query&&!c.category)return'PRODUCT_CRITERIA_REQUIRED';if(!c.place)return'LOCATION_REQUIRED';if((c.lat==null)!==(c.lng==null))return'INVALID_COORDINATES';if(c.from<new Date().toISOString().slice(0,10))return'PAST_PERIOD';return null}
function dbCriteria(c:ReturnType<typeof canonical>,center:{lat:number;lng:number}|null){return{query_text:c.query||null,category:c.category||null,place:c.place||null,center_lat:center?.lat??null,center_lng:center?.lng??null,radius_km:c.radius,start_date:c.from,end_date:c.to,max_total_price:c.maxPrice,min_rating:c.minRating,discount_only:c.discountOnly,locale:c.locale,notification_channel:c.notificationChannel,criteria_hash:criteriaHash({...c,lat:center?.lat??null,lng:center?.lng??null})}}
async function resolveCenter(c:ReturnType<typeof canonical>){if(c.lat!=null&&c.lng!=null)return{lat:c.lat,lng:c.lng};return geocodeSwedishPlace(c.place)}
...
export async function POST(request:Request){
 ...
 const center=await resolveCenter(c);if(!center)return NextResponse.json({error:'LOCATION_NOT_FOUND'},{status:400});
...
export async function PATCH(request:Request){
 ...
 const criteriaFields=['query','category','place','radius','from','to','maxPrice','minRating','discountOnly','locale','notificationChannel','lat','lng'] as const,changesCriteria=criteriaFields.some(key=>Object.prototype.hasOwnProperty.call(body,key)),patch:Record<string,unknown>={updated_at:new Date().toISOString()};
 if(changesCriteria){const merged=canonical({query:body.query??current.query_text??'',category:body.category??current.category??'',place:body.place??current.place??'',radius:body.radius??current.radius_km,from:body.from??current.start_date,to:body.to??current.end_date,maxPrice:body.maxPrice!==undefined?body.maxPrice:current.max_total_price??'',minRating:body.minRating!==undefined?body.minRating:current.min_rating??'',discountOnly:body.discountOnly!==undefined?body.discountOnly:current.discount_only,locale:body.locale??current.locale,notificationChannel:body.notificationChannel??current.notification_channel,lat:body.lat!==undefined?body.lat:current.center_lat,lng:body.lng!==undefined?body.lng:current.center_lng}),invalid=validCriteria(merged);if(invalid)return NextResponse.json({error:invalid},{status:400});const center=await resolveCenter(merged);if(!center)return NextResponse.json({error:'LOCATION_NOT_FOUND'},{status:400});Object.assign(patch,dbCriteria(merged,center))}
