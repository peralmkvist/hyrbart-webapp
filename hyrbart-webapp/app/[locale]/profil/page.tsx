...
export default async function ProfilePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <section className="ds2Page profileDashboard">
      ...
      <div className="profileIdentityCard"><div className="profileIdentityMain"><div className="profileAvatar">P</div><div><h2>Per</h2><p>Danderyd, Sverige</p></div></div><div className="profileStats"><div><strong>12</strong><span>{en ? 'rentals' : 'hyror'}</span></div><div><strong>8</strong><span>{en ? 'reviews' : 'omdömen'}</span></div><div><strong>4,94</strong><span>{en ? 'average rating' : 'snittbetyg'}</span></div></div></div>
