...
type SearchParams = { category?: string; q?: string; from?: string; to?: string; place?: string; radius?: string; nearby?: string; edit?: string; maxPrice?: string; minRating?: string; discountOnly?: string };
...
export default async function ProductsPage({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<SearchParams>}) {
  ...
  const searched = Boolean(query || from || to || place || nearby === '1' || selectedCategory || parsedMaxPrice || parsedMinRating || onlyDiscounted || (radius && radius !== '10'));
  ...
  const keep=(value?:string)=>{const params=new URLSearchParams();if(value)params.set('category',value);if(q)params.set('q',q);if(from)params.set('from',from);if(to)params.set('to',to);if(place)params.set('place',place);if(radius)params.set('radius',radius);if(nearby==='1')params.set('nearby','1');appendFilterParams(params);return `/${locale}/produkter?${params}`};
  const buildProductQuery=()=>{const params=new URLSearchParams();if(q)params.set('q',q);if(selectedCategory)params.set('category',selectedCategory);if(from)params.set('from',from);if(to)params.set('to',to);if(place)params.set('place',place);if(radius)params.set('radius',radius);if(nearby==='1')params.set('nearby','1');appendFilterParams(params);return params};
  ...
  return <div className="pageShell rentPage2">
    ...
    <SearchFilters locale={locale} query={q} from={from} to={to} place={place} nearby={nearby==='1'} category={selectedCategory} radius={radius??'10'} maxPrice={maxPrice} minRating={minRating} discountOnly={onlyDiscounted} categories={categoryOptions} resultCount={filtered.length}/>
    {!locationUnavailable?<SearchAlertCreate locale={locale} query={q} category={selectedCategory} place={place||((nearby==='1')?searchPlace:'')} radius={String(searchRadius)} from={from} to={to} maxPrice={maxPrice} minRating={minRating} discountOnly={onlyDiscounted} resultCount={filtered.length}/>:null}
