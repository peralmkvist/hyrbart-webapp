...
export default async function GenericProductPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const keyFacts: ProductFact[] = [{ key: 'type', icon: <CategoryIcon category={product.category} />, eyebrow: en ? 'Type' : 'Typ', value: typeLabel }];

  if (highlight) keyFacts.push({ key: 'highlight', icon: <CheckIcon />, eyebrow: en ? 'Good to know' : 'Bra att veta', value: highlight });
  else if (included[0]) keyFacts.push({ key: 'included', icon: <CheckIcon />, eyebrow: en ? 'Included' : 'Ingår', value: en ? included[0].en : included[0].sv });
  if (specs[0]) keyFacts.push({ key: 'spec-1', icon: <MeasureIcon />, eyebrow: en ? specs[0].label.en : specs[0].label.sv, value: specs[0].value });
  ...
  return (
    <div className="productPage2">
      ...
      <section className="productHero2">
        <div className="productGallery2">
          {product.images?.length || product.image ? (
            <ProductGallery images={product.images?.length ? product.images : [product.image!]} alt={`${product.brand} ${product.name}`} badge={product.badge} locale={locale} />
        ...
        <div className="productIdentity2">
          ...
          {product.rating != null && product.reviewCount != null ? (
            <div className="productRating2" aria-label={`${product.rating} ${en ? 'out of 5' : 'av 5'}, ${product.reviewCount} ${en ? 'reviews' : 'omdömen'}`}>
              <span aria-hidden="true">★</span><strong>{product.rating.toFixed(1).replace('.', ',')}</strong><span>({product.reviewCount} {en ? 'reviews' : 'omdömen'})</span>
            </div>
          ) : null}
        </div>
      </section>
      ...
      <section className="productSections2">
        {description ? <details className="productAccordion2"><summary><span>{en ? 'Product description' : 'Produktbeskrivning'}</span><ForwardIcon className="productChevron2" /></summary><div className="productAccordionBody2"><p>{description}</p></div></details> : null}
        ...
        {specs.length ? <details className="productAccordion2"><summary><span>{en ? 'Specifications' : 'Specifikationer'}</span><ForwardIcon className="productChevron2" /></summary><div className="productAccordionBody2 productSpecs2">{specs.map((spec) => <div key={spec.label.sv}><span>{en ? spec.label.en : spec.label.sv}</span><strong>{spec.value}</strong></div>)}</div></details> : null}
        {product.guideAvailable ? <Link className="productGuideRow2" href={`/${locale}/produkter/${product.slug}/guide`}><span>{en ? 'User guide' : 'Användarguide'}</span><ForwardIcon /></Link> : null}
      </section>

      {product.hyggloUrl ? <a className="hyggloSecondaryLink2" href={product.hyggloUrl} target="_blank" rel="noreferrer">{en ? 'View existing listing on Hygglo' : 'Visa befintlig annons på Hygglo'}</a> : null}
    </div>
  );
}
