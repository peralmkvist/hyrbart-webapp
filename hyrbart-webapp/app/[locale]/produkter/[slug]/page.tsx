...
export default async function GenericProductPage({params,searchParams}:{params:Promise<{locale:string;slug:string}>;searchParams:Promise<SearchContext>}){
 ...
 {hasRentalContext&&sp.from?<section className="rentalSummaryCard2" aria-label={en?'Selected rental period':'Vald hyresperiod'}>
   <details style={{margin:0}}>
    <summary style={{display:'flex',justifyContent:'space-between',gap:18,alignItems:'flex-start',cursor:'pointer',listStyle:'none'}}>
      <div><span className="summaryEyebrow2">{en?'Selected dates':'Valda datum'}</span><strong className="summaryDates2">{formatRentalDate(sp.from,en)}{sp.to&&sp.to!==sp.from?` – ${formatRentalDate(sp.to,en)}`:''}</strong>{sp.place?<span className="summaryPlace2">{sp.place}{sp.radius?` · ${sp.radius} km`:''}</span>:null}</div>
      {grandTotal!=null?<div style={{textAlign:'right',flex:'0 0 auto'}}><span className="summaryEyebrow2">{en?'Total':'Totalt'}</span><strong className="summaryTotal2">{grandTotal} kr</strong><span className="summaryDetails2">{en?'Show details':'Visa detaljer'} ▾</span></div>:null}
    </summary>
    {days&&regularRental!=null&&rentalCost!=null&&grandTotal!=null?<div className="priceBreakdown2">
      ...
      <div className="priceBreakdownTotal2"><strong>{en?'Total':'Totalt'}</strong><strong>{grandTotal} kr</strong></div>
    </div>:null}
   </details>
   <Link href={editSearchHref} className="editSearchLink2">{en?'Change search':'Ändra sökning'}</Link>
  </section>:null}

 {product.owner?.name?<section className="ownerTrustCard2" aria-label={en?'About the owner':'Om uthyraren'}>
   <div className="ownerIdentity2">
     {product.owner.profileImage?<img className="ownerAvatar2" src={product.owner.profileImage} alt={product.owner.name}/>:<div className="ownerAvatarFallback2">{product.owner.name.slice(0,1)}</div>}
     <div className="ownerNameBlock2"><span>{en?'Rented out by':'Uthyres av'}</span><strong>{product.owner.name}</strong><div className="ownerVerified2"><span className="ownerShield2"><CheckIcon/></span>{en?'Verified with BankID':'Identifierad via BankID'}</div></div>
   </div>
   <div className="ownerMetric2"><span className="ownerMetricIcon2" aria-hidden="true">★</span><div><strong>{ownerRating!=null?ownerRating.toFixed(1).replace('.',','):'–'}</strong><span>{en?'Average rating':'Snittbetyg'}{ownerReviewCount>0?` · ${ownerReviewCount} ${en?'reviews':'omdömen'}`:''}</span></div></div>
   <div className="ownerMetric2"><span className="ownerMessageIcon2" aria-hidden="true">●</span><div><strong>{en?'Usually replies within 1 hour':'Svarar oftast inom 1 timme'}</strong></div></div>
 </section>:null}
