    ...
    return (
      <div className="pageShell genericProductPage">
        ...
        <div className="genericProductHero">
          ...
          <div className="productTitle"><p>{en ? product.typeEn ?? product.type : product.type}</p><h1>{product.brand}<br />{product.name}</h1><p>{product.price}</p></div>
        </div>
  ...
  return (
    <div className="pageShell detailPage productDetailPage">
      ...
      <div className="detailProductVisual productDetailPhotoWrap">
        {product.images?.length || product.image ? <ProductGallery images={product.images?.length ? product.images : [product.image!]} alt={`${product.brand} ${product.name}`} badge={product.badge} locale={locale} /> : <ProductVisual kind="cleaner" accent={product.accent} />}
      </div>
