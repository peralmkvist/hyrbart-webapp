...
export default async function GenericProductPage({params,searchParams}:{params:Promise<{locale:string;slug:string}>;searchParams:Promise<SearchContext>}){
 ...
 const responseTime=product.owner?.responseTimeMinutes ?? 60;
 ...
 {hasRentalContext&&sp.from?<><section className={styles.summaryCard} aria-label={en?'Selected rental period':'Vald hyresperiod'}>
   <details className={styles.priceDetails}>
    <summary style={{display:'flex',justifyContent:'space-between',gap:18,alignItems:'flex-start',cursor:'pointer',listStyle:'none'}}>
      ...
      {pricing?<div style={{textAlign:'right',flex:'0 0 auto'}}><span className={styles.summaryEyebrow}>{en?'Total':'Totalt'}</span><strong className={styles.summaryTotal}>{pricing.total} kr</strong><span className={styles.summaryDetails}><span className={styles.summaryShowDetails}>{en?'Show details':'Visa detaljer'} ▾</span><span className={styles.summaryHideDetails}>{en?'Hide details':'Dölj detaljer'} ▴</span></span></div>:null}
    </summary>
 ...
 {product.owner?.name?<section className={styles.ownerCard} aria-label={en?'About the owner':'Om uthyraren'}>
   ...
   <div className={`${styles.metric} ${styles.ratingMetric}`}><span className={styles.ratingStar} aria-hidden="true">★</span><div className={styles.ratingLine}><strong>{ownerRating!=null?ownerRating.toFixed(1).replace('.',','):'–'}</strong>{ownerReviewCount>0?<span>({ownerReviewCount} {en?'reviews':'omdömen'})</span>:null}</div></div>
   <div className={styles.metric}><span className={styles.messageIcon} aria-hidden="true">●</span><div><strong>{formatResponseTime(responseTime,en)}</strong></div></div>
 </section>:null}
