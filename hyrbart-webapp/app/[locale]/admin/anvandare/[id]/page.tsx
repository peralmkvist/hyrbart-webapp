...
export default async function AdminUserPage({params}:{params:Promise<{locale:string;id:string}>}){
  ...
  const heldPayout=(payouts||[]).filter((p:any)=>p.status==='pending').reduce((sum:number,p:any)=>sum+Number(p.amount||0),0);
  ...
  return <main className="adminPage">
    ...
    <AdminUserOperations userId={id} currentStatus={profile.account_status||'active'} flags={(flags||[]) as any}/>

    <section style={card}><h2 style={heading}>Ekonomisk avstämning</h2><div className="adminStats" style={{gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))'}}><div><b>{captured.toLocaleString('sv-SE')} kr</b><span>Registrerat betalt</span></div><div><b>{refunded.toLocaleString('sv-SE')} kr</b><span>Återbetalat</span></div><div><b>{payoutTotal.toLocaleString('sv-SE')} kr</b><span>Schemalagt/utbetalt</span></div><div><b>{heldPayout.toLocaleString('sv-SE')} kr</b><span>Payout på hold</span></div></div><p style={{color:'var(--muted)',marginBottom:0}}>Beloppen kommer från Hyrbarts interna ledger. Så länge provider är simulation är detta inte bankavstämning.</p></section>

    <section style={card}><h2 style={heading}>Support & risk</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:20}}><div><h3>Privata noteringar</h3>{(notes||[]).map((n:any)=><article key={n.id} style={timelineItem}><small>{new Date(n.created_at).toLocaleString('sv-SE')}</small><p>{n.note}</p></article>)}{!(notes||[]).length?<p>Inga noteringar.</p>:null}</div><div><h3>Riskhistorik</h3>{(flags||[]).map((f:any)=><article key={f.id} style={timelineItem}><strong>{f.severity.toUpperCase()} · {f.status}</strong><small style={{display:'block'}}>{new Date(f.created_at).toLocaleString('sv-SE')}</small><p>{f.reason}</p>{f.resolved_at?<small>Stängd {new Date(f.resolved_at).toLocaleString('sv-SE')}</small>:null}</article>)}{!(flags||[]).length?<p>Inga riskflaggor.</p>:null}</div></div></section>
