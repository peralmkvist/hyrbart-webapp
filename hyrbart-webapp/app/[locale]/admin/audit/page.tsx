...
export default async function AdminAudit({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="adminPage">
    <header className="adminHeader"><div><span>HYRBART SECURITY</span><h1>Audit log</h1><p>Append-only historik över administrativa beslut och ändringar. Rollen vid åtgärdstillfället sparas tillsammans med händelsen.</p><div style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:10}}><Link href={`/${locale}/admin`}>← Adminöversikt</Link>{canExport?<a href="/api/admin/audit/export">Exportera CSV</a>:null}{canManageRoles?<Link href={`/${locale}/admin/roller`}>Roller & behörigheter</Link>:null}</div></div><div className="adminAvatar">H</div></header>
