...
export default async function HostOnboardingPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <section className="hostOnboardingPage">
    ...
    <div className="hostOnboardingSteps">
      <div className={profileReady?'done':''}><i>{profileReady?'✓':'1'}</i><span><strong>{en?'Host profile':'Uthyrarprofil'}</strong><small>{profileReady?(en?'Name and pickup location are ready.':'Namn och utlämningsplats är klara.'):(en?'Add your name and one or more pickup locations.':'Lägg till namn och en eller flera utlämningsplatser.')}</small></span></div>
    ...
    {profileReady?<Link className="hostOnboardingPrimary" href={`/${locale}/vard/annonser/ny`}>{ready?(en?'Create listing':'Skapa annons'):(en?'Create draft':'Skapa utkast')} →</Link>:null}
  </section>;
}
