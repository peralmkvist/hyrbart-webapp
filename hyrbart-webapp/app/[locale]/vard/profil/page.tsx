...
export default async function HostProfilePage({params}:{params:Promise<{locale:string}>}){
  ...
  const [{data:profile},ownerSummary,notificationCount]=await Promise.all([
  ...
  const firstPublishedAt=await getFirstPublishedListingDate(profile?.sanity_profile_id); const hostTenure=formatProfileTenure(firstPublishedAt,locale); const displayName=profile?.display_name||(en?'Hyrbart user':'Hyrbart-användare'); const city=profile?.city; const initial=displayName.trim().charAt(0).toUpperCase()||'H'; const publicProfileHref=`/${locale}/profil/${user.id}`; const insightsHref=`/topsecret/${locale}/vard/profil/insikter`; const revenueHref=`/topsecret/${locale}/vard/profil/intakter`; const reviewCount=ownerSummary.count; const ratingLabel=ownerSummary.overall==null?'–':ownerSummary.overall.toFixed(2).replace('.',','); const unread=notificationCount.count||0;
  ...
  return <section className="ds2Page profileDashboard">
    ...
    <div className="profileInsightGrid"><Link href={revenueHref} className="profileInsightCard profileInsightLink"><h2>{en?'Revenue':'Intäkter'}</h2><p>{en?'Economy and payouts':'Ekonomi och utbetalningar'}</p><div className="profileBars" aria-hidden="true"><i/><i/><i/><i/><i/></div></Link><Link href={insightsHref} className="profileInsightCard profileInsightLink"><h2>{en?'Insights':'Insikter'}</h2><p>{en?'Rental activity':'Uthyrningsdata'}</p><div className="profileRating"><span>★</span><strong>{ratingLabel}</strong></div></Link></div>
