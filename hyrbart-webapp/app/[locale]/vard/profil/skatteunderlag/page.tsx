...
export default async function HostTaxSummaryPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const { data: bookings, error } = await supabase.from('bookings').select('status,rental_price,refund_rental_amount,start_date').eq('owner_id', user.id).gte('start_date', first).lte('start_date', last);
  ...
  return <section className="ds2Page profileSettingsPage">
    ...
    <section className="profileSettingsCard"><div className="profileSettingsCardHeading"><span>{year}</span><h2>{en ? 'Rental summary' : 'Uthyrningsöversikt'}</h2></div><div className="profileTaxMetrics"><div><span>{en ? 'Estimated rental revenue' : 'Uppskattad hyresintäkt'}</span><strong>{currency(revenue)}</strong></div><div><span>{en ? 'Completed rentals' : 'Genomförda uthyrningar'}</span><strong>{completed}</strong></div></div><p>{en ? 'Revenue includes paid, active, returned and completed bookings and subtracts recorded rental refunds. It is not a tax calculation and may differ from amounts that must be reported.' : 'Intäkten omfattar betalda, pågående, återlämnade och slutförda bokningar och drar av registrerade återbetalningar av hyresbelopp. Detta är inte en skatteberäkning och kan skilja sig från belopp som ska deklareras.'}</p></section>
