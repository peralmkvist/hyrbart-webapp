...
begin
  ...
  if p_opened_by is distinct from v_booking.owner_id and p_opened_by is distinct from v_booking.renter_id then
    raise exception 'NOT_PARTICIPANT';
  end if;

  v_allowed := case p_case_type
    when 'problem' then v_booking.status = any(array['accepted','paid','active','returned','completed'])
    when 'damage' then v_booking.status = any(array['active','returned','completed'])
    when 'dispute' then v_booking.status = any(array['paid','active','returned','completed'])
    else false end;
  ...
  values (p_booking_id,p_opened_by,p_case_type,left(trim(p_reason),120),nullif(trim(coalesce(p_description,'')),''),p_amount_claimed,
    case when p_case_type='dispute' then 'under_review' else 'open' end)
  returning * into v_case;

  if v_booking.status = any(array['paid','active','returned']) then
    update public.bookings
      set status='disputed', updated_at=now(), auto_complete_at=null
