...
begin
  ...
  elsif p_resolution_type='split' then v_refund:=greatest(0,coalesce(p_refund_amount,0)); v_payout_amount:=greatest(0,coalesce(p_payout_amount,0)); end if;
  ...
  if v_refund>0 then
    select * into v_payment from public.booking_payments where booking_id=v_booking.id and status in ('captured','partially_refunded','refunded') order by created_at desc limit 1 for update;
    if found then
      update public.booking_payments set refund_amount=least(v_refund,amount),status=case when least(v_refund,amount)>=amount then 'refunded' else 'partially_refunded' end,refunded_at=v_now,updated_at=v_now where id=v_payment.id returning * into v_payment;
    end if;
  end if;
  if v_payout_amount>0 and (v_payout.id is null or v_payout.status<>'paid') then
    ...
    on conflict(booking_id) do update set status='scheduled',amount=excluded.amount,currency=excluded.currency,due_at=excluded.due_at,updated_at=excluded.updated_at returning * into v_payout;
  ...
  v_final_status:=v_booking.status::text;
  if v_booking.status='disputed' then
    v_final_status:=case when p_resolution_type='full_refund' then 'refunded' else 'completed' end;
    update public.bookings set status=v_final_status::public.booking_status,refund_amount=v_refund,completed_at=case when v_final_status='completed' then coalesce(completed_at,v_now) else completed_at end,updated_at=v_now where id=v_booking.id;
  end if;
  update public.booking_cases set status=p_status,resolution=trim(p_decision),resolution_note=trim(p_decision),resolution_type=p_resolution_type,refund_amount=v_refund,payout_amount=v_payout_amount,resolved_at=v_now,resolved_by=p_admin_user_id,updated_at=v_now where id=v_case.id returning * into v_case;
  return jsonb_build_object('case',to_jsonb(v_case),'booking',to_jsonb(v_booking),'resolutionType',p_resolution_type,'refundAmount',v_refund,'payoutAmount',v_payout_amount,'paymentId',v_payment.id,'payoutId',v_payout.id,'finalStatus',v_final_status);
end;
$$;
