...
export function searchAlertUrl(alert: SearchAlertRow) {
  ...
  p.set('from', alert.start_date);
  p.set('to', alert.end_date);
  if (alert.place) p.set('place', alert.place);
  p.set('radius', String(alert.radius_km));
  if (alert.max_total_price != null) p.set('maxPrice', String(alert.max_total_price));
  if (alert.min_rating != null) p.set('minRating', String(alert.min_rating));
  if (alert.discount_only) p.set('discountOnly', '1');
  return `/${alert.locale}/produkter?${p.toString()}`;
}
...
export async function evaluateSearchAlert(alert: SearchAlertRow) {
  ...
  if (alert.start_date < dateToday()) {
    await admin.from('search_alerts').update({ status: 'expired', updated_at: now, last_evaluated_at: now }).eq('id', alert.id);
    return { evaluated: true, expired: true, notifications: 0, matches: 0 };
  }

  const products = await getProducts();
  const reviewSummaries = alert.min_rating != null
    ? await getProductReviewSummaries(products.flatMap(product => product.id ? [product.id] : []))
    : {};
  const [legacyUnavailable, bookedIds, existingResult] = await Promise.all([
    getUnavailableProductSlugs(alert.start_date, alert.end_date),
    getUnavailableProductIds(alert.start_date, alert.end_date),
    admin.from('search_alert_matches').select('*').eq('alert_id', alert.id),
  ]);
  if (existingResult.error) throw existingResult.error;
  const existing = new Map((existingResult.data || []).map(row => [row.product_key, row as MatchRow]));
  const query = normalize(alert.query_text);
  const center = alert.center_lat != null && alert.center_lng != null ? { lat: alert.center_lat, lng: alert.center_lng } : null;
  ...
  for (const product of products) {
    ...
    if (center) {
      if (!product.pickupLocation) continue;
      if (distanceKm(center, { lat: product.pickupLocation.lat, lng: product.pickupLocation.lng }) > alert.radius_km) continue;
    }
    const pricing = calculateRentalPricing(product.price, alert.start_date, alert.end_date, product.rentalPrices, product.discounts);
    if (!pricing) continue;
    ...
    if (alert.min_rating != null) {
      ...
      if (!review?.count || review.overall == null || review.overall < alert.min_rating) continue;
    }
    if (alert.discount_only && !hasDiscount(product)) continue;

    matches += 1;
    seen.add(product.slug);
    const signature = `${pricing.total}|${alert.start_date}|${alert.end_date}`;
    const previous = existing.get(product.slug);
    const shouldNotify = !previous?.is_matching || previous.last_signature !== signature;

    const { data: stored, error: upsertError } = await admin.from('search_alert_matches').upsert({
      alert_id: alert.id,
      ...
      last_matched_at: now,
      updated_at: now,
    }, { onConflict: 'alert_id,product_key' }).select('id').single();
  ...
  for (const [key, row] of existing) {
    if (!seen.has(key) && row.is_matching) {
      await admin.from('search_alert_matches').update({ is_matching: false, updated_at: now }).eq('id', row.id);
    }
  }
...
export async function evaluateActiveSearchAlerts(limit = 100) {
  const admin = createAdminClient();
  const { data, error } = await admin.from('search_alerts').select('*').eq('status', 'active').order('last_evaluated_at', { ascending: true, nullsFirst: true }).limit(limit);
  if (error) throw error;
  let notifications = 0;
  let evaluated = 0;
  let expired = 0;
  for (const row of data || []) {
    const result = await evaluateSearchAlert(row as SearchAlertRow);
    if (result.evaluated) evaluated += 1;
    if (result.expired) expired += 1;
    notifications += result.notifications;
  }
  return { evaluated, expired, notifications };
}
