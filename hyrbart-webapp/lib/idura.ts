...
export const IDURA_COOKIE_NAMES = {
  ...
  attempt: '__Secure-hyrbart-idura-attempt',
  locale: '__Secure-hyrbart-idura-locale',
} as const;
...
export function hasIduraConfig() {
  return Boolean(process.env.IDURA_DOMAIN?.trim() && process.env.IDURA_CLIENT_ID?.trim() && process.env.IDURA_CLIENT_SECRET?.trim());
}

export function getIduraConfig() {
  return {
    domain: requiredEnv('IDURA_DOMAIN').replace(/^https?:\/\//, '').replace(/\/$/, ''),
...
export async function buildAuthorizationUrl(options: { redirectUri: string; state: string; nonce: string; codeChallenge: string }) {
...
export async function exchangeAuthorizationCode(options: { code: string; redirectUri: string; codeVerifier: string }) {
...
async function verifyRs256Signature(options: { signingInput: string; signature: string; jwk: Jwk }) {
  const key = await crypto.subtle.importKey('jwk', options.jwk, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
...
export async function verifyIdToken(options: { idToken: string; discovery: DiscoveryDocument; expectedNonce: string }): Promise<IduraClaims> {
  const { clientId } = getIduraConfig();
  ...
  const jwksResponse = await fetch(options.discovery.jwks_uri, { headers: { Accept: 'application/json' }, cache: 'no-store' });
  ...
  const validSignature = await verifyRs256Signature({ signingInput: `${encodedHeader}.${encodedPayload}`, signature: encodedSignature, jwk });
