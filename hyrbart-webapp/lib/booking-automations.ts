...
import { markLateReturn } from '@/lib/late-returns';
...
function overdueByMinutes(now:number,target:string|null,minutesAfter:number){return Boolean(target&&now>=new Date(target).getTime()+minutesAfter*MINUTE);}
...
export async function processBookingAutomations(){
  ...
  for(const booking of(data??[])as BookingRow[]){try{
    ...
    if(booking.status==='active'&&booking.return_due_at){
      if(overdue(now,booking.return_due_at,24)){
        const scheduledFor=new Date(new Date(booking.return_due_at).getTime()+24*HOUR).toISOString();
        if(await runOnce(booking,'late_return_escalated_24h',scheduledFor,async()=>{const late=await markLateReturn(booking,3);await recordBookingEvent({bookingId:booking.id,eventType:'late_return_escalated_24h',metadata:{actor_role:'system',return_due_at:booking.return_due_at,overdue_minutes:late?.overdue_minutes??null,escalation_level:3}});await notifyBoth(booking,'late_return_escalated_24h','Återlämningen är kraftigt försenad','Objektet är mer än 24 timmar försenat. Uthyraren kan öppna ett ärende från bokningen om hjälp behövs.',`late-return-24h:${booking.id}`);} ))processed++;
      }else if(overdue(now,booking.return_due_at,2)){
        const scheduledFor=new Date(new Date(booking.return_due_at).getTime()+2*HOUR).toISOString();
        if(await runOnce(booking,'return_overdue',scheduledFor,async()=>{const late=await markLateReturn(booking,2);await recordBookingEvent({bookingId:booking.id,eventType:'return_overdue',metadata:{actor_role:'system',return_due_at:booking.return_due_at,overdue_minutes:late?.overdue_minutes??null,escalation_level:2}});await notifyBoth(booking,'return_overdue','Återlämning försenad','Bokningen är fortfarande inte markerad som återlämnad. Förseningen är nu eskalerad.',`return-overdue:${booking.id}`);} ))processed++;
      }else if(overdueByMinutes(now,booking.return_due_at,15)){
        const scheduledFor=new Date(new Date(booking.return_due_at).getTime()+15*MINUTE).toISOString();
        if(await runOnce(booking,'late_return_started',scheduledFor,async()=>{const late=await markLateReturn(booking,1);await recordBookingEvent({bookingId:booking.id,eventType:'late_return_started',metadata:{actor_role:'system',return_due_at:booking.return_due_at,overdue_minutes:late?.overdue_minutes??null,escalation_level:1}});await notifyBoth(booking,'late_return_started','Återlämningen är sen','Återlämningstiden har passerat. Markera återlämningen så snart objektet är tillbaka.',`late-return-started:${booking.id}`);} ))processed++;
      }
    }
    if(booking.status==='returned'&&booking.auto_complete_at&&now>=new Date(booking.auto_complete_at).getTime()){if(await runOnce(booking,'auto_complete',booking.auto_complete_at,async()=>{if(!canBookingTransition('returned','completed','system'))return;const completedAt=new Date().toISOString();const{data:updated,error:updateError}=await admin.from('bookings').update({status:'completed',auto_complete_at:null,completed_at:completedAt,updated_at:completedAt}).eq('id',booking.id).eq('status','returned').select('id,status,completed_at').maybeSingle();if(updateError)throw updateError;if(!updated)return;await recordBookingEvent({bookingId:booking.id,eventType:'booking_auto_completed',metadata:{previous_status:'returned',new_status:'completed',actor_role:'system',completed_at:updated.completed_at}});const payout=await ensureScheduledPayout(booking);await recordBookingEvent({bookingId:booking.id,eventType:'payout_scheduled',metadata:{payout_id:payout.id,amount:payout.amount,currency:payout.currency,provider:payout.provider,simulated:payout.provider==='simulation',actor_role:'system'}});try{await ensureBookingAgreement(booking.id);}catch(agreementError){console.error('Could not generate booking agreement',booking.id,agreementError);}await notifyBoth(booking,'booking_auto_completed','Bokningen är avslutad','Inga problem rapporterades inom 24 timmar efter återlämning.',`auto-complete:${booking.id}`);} ))processed++;}
