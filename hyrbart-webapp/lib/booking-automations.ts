...
async function beginEvent(bookingId:string,eventType:string,scheduledFor:string){
  ...
  const{data,error}=await admin.from('booking_automation_events').insert({booking_id:bookingId,event_type:eventType,event_key:eventKey,scheduled_for:scheduledFor,status:'processing',attempts:1}).select('id').maybeSingle();
  if(!error)return data?.id??null;
  if(error.code!=='23505')throw error;
  const{data:existing,error:loadError}=await admin.from('booking_automation_events').select('id,status,attempts,updated_at').eq('event_key',eventKey).maybeSingle();
  ...
  const updateQuery=admin.from('booking_automation_events').update({status:'processing',attempts:Number(existing.attempts||0)+1,last_error:staleProcessing?'Recovered stale processing event':null,updated_at:new Date().toISOString()}).eq('id',existing.id);
  const{data:retried,error:retryError}=staleProcessing
    ? await updateQuery.eq('status','processing').select('id').maybeSingle()
    : await updateQuery.neq('status','processing').select('id').maybeSingle();
  if(retryError)throw retryError;
  return retried?.id??null;
}
async function finishEvent(id:string,error?:unknown){const admin=createAdminClient();await admin.from('booking_automation_events').update({status:error?'failed':'succeeded',executed_at:error?null:new Date().toISOString(),last_error:error?String(error instanceof Error?error.message:error).slice(0,1000):null,updated_at:new Date().toISOString()}).eq('id',id);}
