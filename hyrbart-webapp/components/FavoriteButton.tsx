...
export default function FavoriteButton({slug,label,compact=false}:{slug:string;label?:string;compact?:boolean}){
 ...
 async function sync(){
  ...
  if(local.includes(slug)) setFavorite(true);
  try{
   ...
   setFavorite(merged.includes(slug));
  }catch{}
 }
 ...
 async function toggle(event:MouseEvent<HTMLButtonElement>){
  ...
  const localNext=next?Array.from(new Set([...current,slug])):current.filter(item=>item!==slug);
  writeLocalFavorites(localNext);
  ...
  try{
   ...
   if(response.ok){
    ...
    if(Array.isArray(data.favorites)){
     const merged=next?Array.from(new Set([...localNext,...data.favorites])):data.favorites.filter(item=>item!==slug);
     writeLocalFavorites(merged);
    }
   }
  }catch{
   // Local storage remains the fallback so favorites still work on this device.
  }finally{
   setSaving(false);
   window.dispatchEvent(new Event(EVENT));
  }
