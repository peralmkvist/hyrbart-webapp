...
export default function FavoriteButton({slug,label,compact=false}:{slug:string;label?:string;compact?:boolean}){
 ...
 async function sync(){try{const response=await fetch('/api/favorites',{cache:'no-store'});if(!response.ok)return;const data=await response.json() as {favorites?:string[]};setFavorite((data.favorites??[]).includes(slug))}catch{}}
 ...
 async function toggle(event:MouseEvent<HTMLButtonElement>){event.preventDefault();event.stopPropagation();if(saving)return;setSaving(true);const next=!favorite;setFavorite(next);try{const response=await fetch('/api/favorites',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({slug,favorite:next})});if(!response.ok)throw new Error();window.dispatchEvent(new Event(EVENT))}catch{setFavorite(!next)}finally{setSaving(false)}}
