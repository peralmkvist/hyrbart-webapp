...
export default function SearchAlertCreate({ locale, query = '', category = '', place = '', radius = '10', from = '', to = '', maxPrice = '', minRating = '', discountOnly = false, resultCount }: Props) {
  ...
  const eligible = Boolean(from && (query.trim() || category) && resultCount <= 3);
  ...
  const location = place ? `${place} · ${radius} km` : (en ? `Within ${radius} km` : `Inom ${radius} km`);
  ...
  return <section style={{margin:'14px 0 22px',padding:'18px',border:'1px solid var(--line)',borderRadius:20,background:'#fff'}} aria-label={en?'Watch this search':'Bevaka sökningen'}>
    ...
    {open ? <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid var(--line)'}}>
      {saved ? <div><strong>{en?'Watch saved':'Bevakningen är sparad'}</strong><p style={{color:'var(--muted)'}}>{en?'We will re-check availability and the mandatory total price before notifying you. A notification is not a reservation or guarantee.':'Vi kontrollerar tillgänglighet och obligatoriskt totalpris på nytt innan en notis skickas. En notis är inte en reservation eller garanti.'}</p><Link href={`/${locale}/profil/bevakningar`}>{en?'Manage watches':'Hantera bevakningar'} →</Link></div> : <>
        ...
        {minRating ? <p style={{fontSize:13,color:'var(--muted)'}}>★ {minRating}+ · {discountOnly?(en?'discount required':'rabatt krävs'):en?'all price types':'alla pristyper'}</p> : null}
