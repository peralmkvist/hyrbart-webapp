...
export default function FavoritesGrid({locale,products}:{locale:string;products:FavoriteProduct[]}){
 ...
 async function sync(){try{const response=await fetch('/api/favorites',{cache:'no-store'});if(!response.ok)return;const data=await response.json() as {favorites?:string[]};setSlugs(data.favorites??[])}finally{setLoading(false)}}
