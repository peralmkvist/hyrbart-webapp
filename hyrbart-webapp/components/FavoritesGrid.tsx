...
function readLegacyLocalFavorites(){try{const raw=window.localStorage.getItem(STORAGE_KEY);const parsed=raw?JSON.parse(raw):[];return Array.isArray(parsed)?parsed.filter((value):value is string=>typeof value==='string'):[];}catch{return [] as string[]}}
...
export default function FavoritesGrid({locale,products}:{locale:string;products:FavoriteProduct[]}){
 ...
 const [slugs,setSlugs]=useState<string[]>([]),[loading,setLoading]=useState(true),[failed,setFailed]=useState(false),[authenticated,setAuthenticated]=useState(true);
 ...
 useEffect(()=>{if(!filterOpen)return;const close=(event:PointerEvent)=>{if(filterWrapRef.current&&!filterWrapRef.current.contains(event.target as Node))setFilterOpen(false)};document.addEventListener('pointerdown',close);return()=>document.removeEventListener('pointerdown',close)},[filterOpen]);
 ...
 async function removeFavorite(slug:string){setRemoving(slug);try{const response=await fetch('/api/favorites',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({slug,favorite:false})});if(!response.ok)throw new Error();setSlugs(current=>current.filter(item=>item!==slug));window.dispatchEvent(new Event(EVENT));}finally{setRemoving(null)}}
 const header=<header className="favoritesHeader2"><h1>{en?'Favorites':'Favoriter'}</h1><div className="favoritesToolbar2"><div ref={filterWrapRef} className="favoritesFilterWrap2"><button type="button" className={`favoritesToolButton2 ${filter!=='all'?'active':''}`} aria-label={en?'Filter favorites':'Filtrera favoriter'} aria-expanded={filterOpen} onClick={()=>setFilterOpen(value=>!value)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 6h16M7 12h10M10 18h4"/></svg></button>{filterOpen&&<div className="favoritesFilterMenu2"><button type="button" className={filter==='all'?'selected':''} onClick={()=>{setFilter('all');setFilterOpen(false)}}>{en?'All':'Alla'}</button>{types.map(type=><button type="button" key={type} className={filter===type?'selected':''} onClick={()=>{setFilter(type);setFilterOpen(false)}}>{type}</button>)}</div>}</div><button type="button" className={`favoritesEditButton2 ${editing?'active':''}`} onClick={()=>setEditing(value=>!value)}>{editing?(en?'Done':'Klar'):(en?'Edit':'Redigera')}</button></div></header>;
 if(loading)return <>{header}<p className="ds2Intro favoritesStatus2" role="status">{en?'Loading favorites…':'Laddar favoriter…'}</p></>;
 if(failed)return <>{header}<div className="favoritesStatus2" role="alert"><p className="ds2Intro">{en?'We could not load your favorites.':'Vi kunde inte ladda dina favoriter.'}</p><button type="button" className="launchStateButton" onClick={()=>void sync()}>{en?'Try again':'Försök igen'}</button></div></>;
