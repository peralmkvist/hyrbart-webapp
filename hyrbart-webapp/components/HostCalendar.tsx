...
export default function HostCalendar({ locale }: { locale: string }) {
  ...
  const months=useMemo(()=>Array.from({length:49},(_,i)=>addMonths(todayMonth,i-24)),[todayMonth]);
  ...
  return <section className="hostCalendarPage">
    ...
    {view==='month'?<><div className="hostCalendarMonthsScroll">{months.map(calendarMonth=>{const monthLabel=new Intl.DateTimeFormat(en?'en-GB':'sv-SE',{month:'long',year:'numeric'}).format(calendarMonth);const cells=monthDays(calendarMonth);const currentMonth=calendarMonth.getFullYear()===todayMonth.getFullYear()&&calendarMonth.getMonth()===todayMonth.getMonth();return <section className="hostCalendarMonthSection" data-current-month={currentMonth?'true':undefined} key={`${calendarMonth.getFullYear()}-${calendarMonth.getMonth()}`}><strong className="hostCalendarMonthTitle">{monthLabel}</strong><div className="hostCalendarCard hostCalendarStackedCard"><div className="hostCalendarWeekdays">{weekdayLabels.map(label=><span key={label}>{label}</span>)}</div><div className="hostCalendarGrid hostCalendarStackedGrid">{cells.map((date,index)=>{if(!date)return <span className="hostCalendarBlankDay" key={`blank-${index}`}/>;const dateIso=iso(date),dayBlocks=blocksOnDate(dateIso),selected=inSelection(dateIso),booking=dayBlocks.find(b=>b.status==='booked'||b.status==='reserved'),cls=`hostCalendarDay ${dateIso===todayIso?'today ':''}${selected?'selected ':''}${dayBlocks.length?'blocked ':''}`;return <button type="button" key={dateIso} className={cls} onClick={()=>booking?openBooking(booking):selectDate(dateIso)} aria-label={booking?(en?'Open booking':'Öppna bokning'):dateIso} style={selected?{background:'var(--accent)',color:'var(--ink)'}:dayBlocks.length?{background:'#f0f0ed'}:undefined}><span>{date.getDate()}</span>{dayBlocks.length>0&&dateIso!==todayIso&&<i style={{width:6,height:6,borderRadius:'50%',background:dayBlocks.some(b=>b.status==='booked')?'var(--accent)':dayBlocks.some(b=>b.status==='reserved')?'#111':'#777',position:'absolute',bottom:5}}/>}</button>})}</div></div></section>})}</div><div className="hostCalendarLegend"><span><i className="available"/>{en?'Available':'Tillgänglig'}</span><span><i className="blocked"/>{en?'Blocked':'Blockerad'}</span><span><i className="booked"/>{en?'Booked / reserved':'Bokad / reserverad'}</span></div><CalendarTodayJump active={view==='month'} /></>:<div className="hostCalendarList">{upcomingBookings.length?upcomingBookings.map(block=><button type="button" onClick={()=>openBooking(block)} key={block.id} style={{display:'block',width:'100%',padding:'14px 4px',border:0,borderBottom:'1px solid var(--line)',background:'transparent',color:'inherit',textAlign:'left',font:'inherit',cursor:'pointer'}}><strong>{products.find(p=>p.id===block.productId)?.label||'Annons'}{block.note?` · ${block.note}`:''}</strong><div style={{color:'var(--muted)',marginTop:4}}>{block.from}{block.to!==block.from?` – ${block.to}`:''} · {statusLabel(block)}</div></button>):<div className="hostCalendarListEmpty"><ListIcon/><strong>{en?'No upcoming bookings':'Inga kommande bokningar'}</strong></div>}</div>}
